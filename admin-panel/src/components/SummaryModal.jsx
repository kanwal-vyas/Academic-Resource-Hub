  ...
  const renderSummary = (text) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith('###')) {
        ...
        elements.push(<h3 key={index} dangerouslySetInnerHTML={{ __html: parseBold(content) }} />);
      } 
      // Bullet points (* or -)
      else if (trimmedLine.startsWith('* ') || trimmedLine.startsWith('- ')) {
        const content = trimmedLine.replace(/^[\*\-]\s*/, '');
        currentList.push(<li key={index} dangerouslySetInnerHTML={{ __html: parseBold(content) }} />);
      } 
      // Regular paragraphs
      else {
        flushList();
        elements.push(<p key={index} dangerouslySetInnerHTML={{ __html: parseBold(trimmedLine) }} />);
      }
    });
